...
import playerClubs from '../../components/playerClubs1';

function ItemOrderingPage1() {
  ...
  /*useEffect(() => {
    const savedPoints = localStorage.getItem('ballonDorPoints');
    const savedPositions = localStorage.getItem('ballonDorPositions');
  ...
  useEffect(() => {
    localStorage.setItem('ballonDorPoints', JSON.stringify(pointAssignments));
  }, [pointAssignments]);

  useEffect(() => {
    localStorage.setItem('ballonDorPositions', JSON.stringify(positionAssignments));
  }, [positionAssignments]);*/
    ...
    <div style={{ padding: '20px', textAlign: 'center' }}>
      ...
      {pointValues.map((pt) => (
        <div key={pt} style={{ margin: '8px 0' }}>
          ...
          >
            <option value="" style={{ fontFamily: 'Figtree', textAlign: 'center' }}>-- {t('selectPlayer')} --</option>
            {items.map((item) => (
      ...
      <h2 style={{ marginTop: '30px' }}>{t('Apoints2')}</h2>
      {items
        .filter((item) => !usedInPoints.includes(item))
        .map((item) => (
      ...
      <div className="back-button-container">
        <button className="back-button">
          <Link to="/Ballon d'Or/masculin">{t('back')}</Link>
        </button>
      </div>      
      ...
      {showResult && (
        <>
          ...
          <button style={{ marginTop: '10px', fontFamily: 'Figtree' }}>
            <Link to="/Trophée Kopa/masculin">{t('next')}</Link>
          </button>
        </>
      )}
...
export default ItemOrderingPage1;