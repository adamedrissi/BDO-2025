...
function ItemOrderingPage4() {
  ...
  /*useEffect(() => {
    const savedTop5 = localStorage.getItem('jcTrophyTop5');
    if (savedTop5) {
  ...
  useEffect(() => {
    localStorage.setItem('jcTrophyTop5', JSON.stringify(items));
  }, [items]);*/
    ...
    <div style={{ textAlign: 'center', padding: '20px' }}>
      ...
      <div className="back-button-container">
        <button className="back-button">
          <Link to="/Trophée Johan Cruyff/masculin">{t('back')}</Link>
        </button>
      </div>     
      ...
      {showResult && (
        <>
          ...
          <button style={{ marginTop: '10px', fontFamily: 'Figtree' }}>
            <Link to="/World XI/masculin">{t('next')}</Link>
          </button>
        </>
      )}
...
export default ItemOrderingPage4;