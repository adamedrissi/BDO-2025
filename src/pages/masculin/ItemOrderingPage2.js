...
function ItemOrderingPage2() {
  ...
  /*useEffect(() => {
    const savedTop10 = localStorage.getItem('kopaTop10');
    if (savedTop10) {
  ...
  useEffect(() => {
    localStorage.setItem('kopaTop10', JSON.stringify(items));
  }, [items]);*/
    ...
    <div style={{ textAlign: 'center', padding: '20px' }}>
      ...
      <div className="back-button-container">
        <button className="back-button">
          <Link to="/Trophée Kopa/masculin">{t('back')}</Link>
        </button>
      </div>      
      ...
      {showResult && (
        <>
          ...
          <button style={{ marginTop: '10px', fontFamily: 'Figtree' }}>
            <Link to="/Trophée Yachine/masculin">{t('next')}</Link>
          </button>
        </>
      )}
...
export default ItemOrderingPage2;