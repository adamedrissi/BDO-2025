...
function PlayerAwardImage2f({ top5 }) {
  const { t } = useTranslation();
  const canvasRef = useRef(null);
  const backgroundImage = process.env.PUBLIC_URL + '/results2f.png';
  const shareText = `TROPHÉE KOPA FÉMENIN 2025 - Top 5\n\n` + top5.map((name, index) => `${index + 1}. ${typeof name === 'string' ? name : name.name}`).join('\n') + `\n\nhttps://ballondor2025.netlify.app/Troph%C3%A9e%20Kopa/femenin`;
  const yourURL = "https://ballondor2025.netlify.app/Troph%C3%A9e%20Kopa/femenin";
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const topCoords = [
        [90, 133], [105, 291], [120, 410], [150, 458], [150, 493]
      ];
  ...
  }, [backgroundImage, top5]);

  const downloadImage = () => {
    const link = document.createElement('a');
    link.download = 'kopa2025f.png';
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '20px' }}>
      <canvas ref={canvasRef} width={1211} height={506} style={{ border: '1px solid #ccc' }} />
      <br />
...
export default PlayerAwardImage2f;