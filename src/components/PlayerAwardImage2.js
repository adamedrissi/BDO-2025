...
function PlayerAwardImage2({ top10 }) {
  const { t } = useTranslation();
  const canvasRef = useRef(null);
  const backgroundImage = process.env.PUBLIC_URL + '/results2.png';
  const shareText = `TROPHÉE KOPA 2025 - Top 10\n\n` + top10.map((name, index) => `${index + 1}. ${typeof name === 'string' ? name : name.name}`).join('\n') + `\n\nhttps://ballondor2025.netlify.app/Troph%C3%A9e%20Kopa/masculin`;
  const yourURL = "https://ballondor2025.netlify.app/Troph%C3%A9e%20Kopa/masculin";
...
export default PlayerAwardImage2;