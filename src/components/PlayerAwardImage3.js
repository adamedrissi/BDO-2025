...
function PlayerAwardImage3({ top10 }) {
  const { t } = useTranslation();
  const canvasRef = useRef(null);
  const backgroundImage = process.env.PUBLIC_URL + '/results3.png';
  const shareText = `TROPHÉE YACHINE 2025 - Top 10\n\n` + top10.map((name, index) => `${index + 1}. ${typeof name === 'string' ? name : name.name}`).join('\n') + `\n\nhttps://ballondor2025.netlify.app/Troph%C3%A9e%20Yachine/masculin`;
  const yourURL = "https://ballondor2025.netlify.app/Troph%C3%A9e%20Yachine/masculin";
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const topCoords = [
        [90, 138], [105, 295], [120, 414], [150, 462], [150, 498],
        [150, 533], [150, 568], [150, 604], [150, 640], [150, 676]
      ];
  ...
  }, [backgroundImage, top10]);

  const downloadImage = () => {
    const link = document.createElement('a');
    link.download = 'yachine2025.png';
...
export default PlayerAwardImage3;